...
class HrController extends Controller
{
    ...
    public function index()
    {
        ...
        $level_account_hr = DB::table('level_account')
                            ...
                            ->first();
        if($level_account_hr){
            $expire_time = Carbon::parse($level_account_hr->expire_time);
            return view('hr_view.index', compact('hr', 'num_cv_accepted', 'num_cv_not_seen', 'num_posts', 'num_all_cv', 'level_account_hr','expire_time'));
        }
        // $now = Carbon::now();
                            
        // $diffInDays = $now->diffInDays($expire_time);
        // $diffInHours = $now->copy()->addDays($diffInDays)->diffInHours($expire_time);           
        // $level_account_hr->expire_time = 'Hết hạn sau: ' . $diffInDays . ' ngày ' . $diffInHours . 'h ';
        // //if expire time < 0 delete
        // if($diffInDays < 0){
        //       DB::table('level_account')
        //         ->where('hr_id',session()->get('id_hr'))
        //         ->delete();
        // }
