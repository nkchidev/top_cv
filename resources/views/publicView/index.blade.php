                    ...
                    <div class="col-md-8">
                                            ...
                                            <div class="company-content">
                                                <div class="company_item">
                                                    <img src="{{ asset('storage/' . $hot_company->logo) }}" alt=""
                                                        class="img_hot_company">
                                                    <div class="company_decription">
                                                        <h4 class="company_name"><a
                                                                href="{{ route('company.detail', $hot_company->id) }}">{{ $hot_company->name }}</a>
                                                        </h4>
                                                        <p class="company_address">{{ $hot_company->address }}</p>
                                                        <p class="company_phone">Số điện thoại:
                                                            {{ $hot_company->phone }}</p>
                                                        <div class="detail">
                                                            <a href="{{ route('company.detail', $hot_company->id) }}">Xem
                        ...
                        <div class="col-md-4">
                            <ul class="logo_company">
                                <input type="hidden" value="{{ $hot_company->id }}">
                                @foreach ($hot_companies as $hot_company)
                                    <li class="list_img_logo"> <img src="{{ asset('storage/' . $hot_company->logo) }}"
                                            alt=""> </li>
                                @endforeach
                            </ul>
                        ...
                        <div class="job-content">
                            <div class="list_hot_job">
                                @foreach ($hot_jobs as $hot_job)
                                    ...
                                    <div class="job_title"><a
                                            href="{{ route('post.detail', [$hot_job->id, $hot_job->slug]) }}">{{ $hot_job->title }}</a>
                                    </div>
                                    <div class="salary">
                                        {{ $hot_job->min_salary }}{{ $hot_job->unit_money }}-{{ $hot_job->max_salary }}{{ $hot_job->unit_money }}
    ...
    <div class="main_content">
        <div class="sum_job">
            ...
            @foreach ($num_jobs_by_city as $num_job_by_city)
                <div class="total_job" style="color:black;" data-city="{{ $num_job_by_city->city }}">
                    {{ $num_job_by_city->city }}({{ $num_job_by_city->total }})</div>
            @endforeach
        </div>
        <div class="post">
                        ...
                        <div class="post_introduce_company">
                            ...
                            <div class="detail_introduce_company">
                                <div class="name_company">
                                    <a href="{{route('post.detail',[$review_company->id,$review_company->slug])}}">{{ $review_company->name }}</a>
                                </div>
                                <div class="describe">
                                    <a href="{{route('post.detail',[$review_company->id,$review_company->slug])}}">{{ $review_company->title }}</a>
                                    <div class="salary-time_post">
                                        <span><i
                                                class="fa-solid fa-money-bill-1-wave"></i>{{ $review_company->min_salary }}-{{ $review_company->max_salary }}{{ $review_company->unit_money }}</span>
                                        <span><i
                                                class="fa-sharp fa-solid fa-clock"></i>{{ $review_company->expired_date() }}
                                            hết hạn</span>
                                    </div>
                                    <div class="detail">
                                        <button>
                                            <a href="{{route('post.detail',[$review_company->id,$review_company->slug])}}">
                                                Tìm hiểu ngay
                                            </a>
                                        </button>
                                    </div>
                ...
                $list_total_job_by_city[i].addEventListener('click', function() {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $(document).on('click', '.list_posts>.post_by_city>nav>ul a',
                                function(e) {
                                    e.preventDefault();
                                    let page = $(this).attr('href').split('page=')[1]
