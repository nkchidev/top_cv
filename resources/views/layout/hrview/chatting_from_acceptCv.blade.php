<head?>
    ...
    <div id="frame">
        <div id="sidepanel">
            <div id="profile">
                <div class="wrap">
                    <img id="profile-img" src="{{ asset('storage/images/' . session('avatar')) }}" class="online"
                        alt="" />
                    <p>{{ session('hr_name') }}</p>
                        ...
                        @foreach ($rooms_chat_of_hr as $room_chat)
                            @if ($room_chat->from == 0)
                                @if ($room_chat->status == 0)
                                    <li class="contact" style="background-color:lightslategray;margin-top:5px"
                                        value="{{ $room_chat->room_id }}">
                                    @else
                            ...
                            <div class="wrap">
                                ...
                                <img src="{{ asset('storage/' . $room_chat->applicant_avatar) }}" alt=""
    ...
    <div class="content">
        ...
        <div class="messages" id="messages">
            @if ($text_message->count() == 0)
                <p style="text-align:center;margin-top:20px;font-size:18px" id="none_message">Hãy bắt đầu cuộc trò
                    chuyện bằng một lời chào 😍</p>
                <ul id="list_messages">
            ...
            @else
                <ul id="list_messages">
                    @foreach ($text_message as $message)
                        ...
                        @else
                            <li class="replies">
                                <img src="{{ asset('storage/' . $message->applicant_avatar) }}" alt=""
                                    style="width:30px;height:30px;" />
                                <p>{{ $message->text_message }}</p>
                            </li>
                        @endif
    ...
    <script>
        ...
        hr_id = '{{ session('id_hr') }}';
        let roomid = '{{ $room_id }}';
        let pusher2 = new Pusher('f159bf8e622a9a565b4f', {
            cluster: 'ap1',
            encrypted: true,
        ...
        let private_message_each_room = pusher2.subscribe('Chatting-channel-of-hr.' + roomid + '.' + hr_id);
        private_message_each_room.bind('Chatting-hr-event', function(data) {
            $('.messages ul').append(
                '<li class="replies"><img src="{{ asset('storage/' . $applicant->avatar) }}" alt="" style="width:30px;height:30px"/><p>' +
                data.text_message + '</p></li>');
            scrollToBottom();
            $.ajax({
    ...
    <script>
        ...
        $('.submit').click(function() {
            let text_message = $('.message-input input').val();
            let room_id = {{ $room_id }};
            ...
            $.ajax({
                ...
                dataType: "json",
                success: function(data) {
                    let company_logo = data.company_info.company_logo;
