        ...
        @if ($rooms_chat_of_hr->count() == 0)
            <div class="without_contact" style="position: absolute;top: 45%;left: 52%;">
                <img src="https://candidate.topcvconnect.com/img/no-conversation.8966b27a.png" alt=""
                    style="height:200px">
                <p style="margin-left:-20px;margin-top:10px;font-size: 18px;">Bạn chưa liên hệ với bất kì ứng cử
                    viên
                    nào!!!</p>
            </div>
        @else
...
@if ($rooms_chat_of_hr->count() == 0)
    <div class="without_contact" style="position: absolute;top: 45%;left: 52%;">
        <img src="https://candidate.topcvconnect.com/img/no-conversation.8966b27a.png" alt=""
            style="height:200px">
        <p style="margin-left:-20px;margin-top:10px;font-size: 18px;">Bạn chưa liên hệ với bất kì ứng cử viên nào!!!
        </p>
        <div style="text-align:center">
            <a href="{{ route('list.applicants.accepted') }}">
                <button id="apply_btn" style="color:white;border-color: #00b14f;border-radius: 10px;padding: 15px;">Liên
                    lạc
                    ngay</button>
            </a>
        </div>
                ...
                for (var i = 0; i < list_contact.length; i++) {
                    list_contact[i].addEventListener('click', function() {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                console.log('data.data_html.hr', data.data_html);
                                $('.content').html(data.data_html);
                            },
							error: function(error) {
								console.log('error from chat_hr', error);
							}
                        });
