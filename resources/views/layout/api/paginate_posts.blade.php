@foreach ($posts as $post)
    <div class="post_item" style="{{ $post->borderpost == 1 ? 'border:solid 5px green;' : '' }}">
        <div class="row">
            ...
            <div class="col-md-8 img-title_job-description">
                ...
                <div class="description-post">
                    <h3 class="title-job"><a
                            href="{{ route('post.detail', [$post->id, $post->slug]) }}">{{ $post->title }}</a></h3>
                    <div class="company-name">{{ $post->company_name }}</div>
                    <span class="btn-introduce-post" style="color:black;">{{ $post->min_salary }}
                        {{ $post->unit_money }} -
                        {{ $post->max_salary }} {{ $post->unit_money }}</span>
