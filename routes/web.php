...
Route::get('/auth/callback/{provider}', function ($provider) {
    ...
    if ($query) {
        if ($query->status_account == 0) {
            return redirect()->route('login')->with('error_active_account', 'Tài khoản đã bị khóa');
        }
...
Route::group([
    ...
    Route::get('update/status/messages/applicant', [MessageController::class, 'updateStatusMessage_applicant'])->name('update.status.message.applicant');
...
Route::get('list/companies', [CompaniesController::class, 'list_companies'])->name('list.companies');
Route::get('company/detail/{id}', [CompaniesController::class, 'company_detail'])->name('company.detail');
...
Route::group([
    ...
    Route::get('/admin', [adminController::class, 'index_admin_view'])->name('admin.index.view');
